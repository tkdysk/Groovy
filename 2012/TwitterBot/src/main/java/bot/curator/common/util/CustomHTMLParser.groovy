...
class CustomHTMLParser {
	...
	public List<String> getContents() throws Exception {
		...
		doTraverse ( root, 1, contents, nodeNames )
...
//	}
	...
	private def doTraverse ( Node parent, int indent, List<String> contents, List<String> nodeNames ) {
		Node child = parent.getFirstChild();
		while ( child != null ) {
			String nodeName = child.getNodeName();
			if(nodeName=="BODY" && nodeNames.size()>0 && nodeNames.get(nodeNames.size()-1)=="HEAD") {
				contents = []
				nodeNames = []
				return
			}
			...
			// ------------------------------------------------
			doTraverse ( child, indent + 1, contents, nodeNames );
